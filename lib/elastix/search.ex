defmodule Elastix.Search do
  ...
  def search(elastic_url, index, types, data) when is_list(data), do: search(elastic_url, index, types, data, [])

  def search(elastic_url, index, types, data), do: search(elastic_url, index, types, data, [])
  ...
  def search(elastic_url, index, types, data, query_params, options) when is_list(index) do
    search(elastic_url, Enum.join(index, ","), types, data, query_params, options)
  end

  def search(elastic_url, index, types, data, query_params, options) when is_list(data) do
    data =
      data
    ...
    elastic_url
    |> prepare_url(make_path(index, types, query_params, "_msearch"))
    |> HTTP.post(data, [], options)
  end

  def search(elastic_url, index, types, data, query_params, options) do
    elastic_url
    |> prepare_url(make_path(index, types, query_params))
  ...
  @spec count(elastic_url :: String.t(), index :: String.t(), types :: list, data :: map) ::
          HTTP.resp()
  def count(elastic_url, index, types, data), do: count(elastic_url, index, types, data, [])
  ...
  def count(elastic_url, index, types, data, query_params, options \\ []) do
    HTTP.post(elastic_url <> make_path(index, types, query_params, "_count"), JSON.encode!(data), [], options)
  end
